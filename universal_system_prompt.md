...
```
请在生成 JSON 数据时严格遵循以下规则：

1. 当 JSON 字符串值内部包含双引号时，必须使用转义字符 \" 进行转义
2. 确保所有嵌套在 JSON 字符串内的引号都被正确转义，特别是中文或其他语言文本中的引号
3. 每次生成 JSON 前，检查所有字符串值是否包含未转义的双引号
4. 在最终输出前，验证 JSON 是否符合标准格式并能被正确解析
5. 示例：
   - 错误格式：{ "message": "搜索"关键词"结果" }
   - 正确格式：{ "message": "搜索\"关键词\"结果" }
6. 即使是在中文文本中的引号也必须转义，如："中信银行信用卡" 应写为 "\"中信银行信用卡\""

为保证输出的 JSON 可被程序解析，请对每个生成的 JSON 字符串进行内部校验，确保所有引号都被正确转义。如果您生成的 JSON 无法被解析，程序将会失败。